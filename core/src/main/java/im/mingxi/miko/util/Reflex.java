...
public class Reflex {
    ...
    public static Class<?> loadClass(@NonNull String clz) {
        try {
            ...
            /*
             * Attempt to return to class which from module
             */
            return _loadClass(clz);
        }
    }
    ...
    public static MethodMatcher findMethodObj(Object obj) {
        return MethodMatcher.create(obj.getClass());
    }
    ...
    public static FieldMatcher findFieldObj(Object obj) {
        return FieldMatcher.create(obj.getClass());
    }
                ...
                for (Class<?> clz : this.reflexMethod.className) {
                    for (Method m : clz.getDeclaredMethods()) {
                        ...
                        if (this.reflexMethod.getMethodName() != null
                                && !this.reflexMethod.getMethodName().equals(m.getName()))
                            temp.remove(m);
                        if (this.reflexMethod.getReturnType() != null
                                && !this.reflexMethod.getReturnType().equals(m.getReturnType()))
                            temp.remove(m);
                        if (this.reflexMethod.getParamsLength() != -1
                                && this.reflexMethod.getParamsLength() != m.getParameterCount())
                            temp.remove(m);
                        if (this.reflexMethod.getParams() != null) {
                            if (this.reflexMethod.getParams().length != m.getParameterCount())
                                temp.remove(m);
                            else {
                                int i = 0;
                                for (Class<?> clazz : this.reflexMethod.getParams()) {
                                    if ((!clazz.equals(m.getParameterTypes()[i]))
                ...
                for (Class<?> clz : this.reflexField.className) {
                    for (Field f : clz.getDeclaredFields()) {
                        ...
                        if (this.reflexField.getFieldName() != null
                                && !this.reflexField.getFieldName().equals(f.getName()))
                            temp.remove(f);
                        if (this.reflexField.getReturnType() != null
                                && !this.reflexField.getReturnType().equals(f.getType()))
                            temp.remove(f);
                    }
                }
                if (temp.size() == 0) {
